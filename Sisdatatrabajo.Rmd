...
```
6. Observamos que se encuentran los valores, pero eliminamos las columnas innecesarias como Codigo. Para ello separo y elimino
...
```{r}
Sis$Regiones=str_split(string = Sis$Regiones,
         pattern = ',',
         simplify = T)[,1]
```
...
Sis$Provincia=str_split(string = Sis$Provincia,
         ...
         simplify = T)[,2]
```
9. Ahora con los datos necesarios, aplicamos la funcion rowSums para obtener el total de los datos encuestados por Provincia
```{r}

Sis$`Total`= rowSums(Sis[ ,2:3])
```

10. Para terminar ,como la tabla obtenida se encuentra desordenada, ordeno esta Data frame ,que tiene distintos objetos tanto numericos como caracteres.
